...
class Uniform_Sampling:
    """SARAH variance reduction with hard thresholding and uniform mini-batch sampling."""
    ...
    def fit(self, centers, targets, x_true, s, batch_size=70, m=None, seed=None, verbose=False):
        """
            Parameters:
            centers : sample rows a_i
        ...
        for it in range(self.max_iter):
            if it > 0 and rng.integers(m) != 0:
                ...
                grad_diff = batch_grad_diff(self.x, xp, Xb, weights=None)
                v = v + grad_diff
